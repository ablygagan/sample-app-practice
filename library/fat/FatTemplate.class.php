			...
			foreach ($arrCommonfiles as $fl) {
				...
				if ($mergeFiles) {
					...
					$str .= '<link rel="stylesheet" type="text/css"
						href="' . FatUtility::generateUrl ( 'JsCss', 'cssCommon', array (), $use_root_url, false ) . '&f=' . rawurlencode ( $fl ) . '&min=0&sid=' . $time . '" />' . "\n";
				}
			}

			if ($mergeFiles) {
				$str .= '<link rel="stylesheet" type="text/css"
						href="' . FatUtility::generateUrl ( 'JsCss', 'cssCommon', array (), $use_root_url, false ) . '&min=1&sid=' . $last_updated . '" />' . "\n";
			}
		}
		...
		if ( count($this->arr_page_css) > 0 ) {
			...
			foreach ($this->arr_page_css as $val){
				...
				if ( $mergeFiles ) {
					...
					$str .= '<link rel="stylesheet" type="text/css"
							href="' . FatUtility::generateUrl('JsCss', 'css', array(), $use_root_url, false) . '&f=' . rawurlencode($val) . '&min=0&sid=' . $time . '" />' . "\n";
				}
			}
			if ( $mergeFiles ) {

				$str .= '<link rel="stylesheet" type="text/css"
						href="' . FatUtility::generateUrl('JsCss', 'css', array(), $use_root_url, false) . '&f=' . rawurlencode(implode(',', $this->arr_page_css)) . '&min=1&sid=' . $last_updated . '" />' . "\n";
			}
		}
		/* Include CSS Ends */
...
// 			$dir = opendir($pth);
			...
			foreach ($arrCommonfiles as $fl) {
				...
				if ( $mergeFiles ) {
					...
					$str .= '<script type="text/javascript" language="javascript"
							src="' . FatUtility::generateUrl('JsCss', 'jsCommon', array(), $use_root_url, false) . '&f=' . rawurlencode($fl) . '&min=0&sid=' . $time . '"></script>' . "\n";
				}
			}

			if ( $mergeFiles ) {
				$str .= '<script type="text/javascript" language="javascript"
							src="' . FatUtility::generateUrl('JsCss', 'jsCommon', array(), $use_root_url, false) . '&min=0&sid=' . $last_updated . '"></script>' . "\n";
			}
		}
		if ( count($this->arr_page_js) > 0 ) {
			...
			if ( $mergeFiles ) {
				$str .= '<script type="text/javascript" language="javascript"
						src="' . FatUtility::generateUrl('JsCss', 'js', array(), $use_root_url, false) . '&f=' . rawurlencode(implode(',', $this->arr_page_js)) . '&min=1&sid=' . $last_updated . '" ></script>' . "\n";
			}
		}
		/* Include JS Ends */
	...
	public function addJs($file){
		...
		if (!in_array($file, $this->arr_page_js)) $this->arr_page_js[] = $file;
	}
	...
	public function addCss($file){
		...
		if (!in_array($file, $this->arr_page_css)) $this->arr_page_css[] = $file;
	}
}
